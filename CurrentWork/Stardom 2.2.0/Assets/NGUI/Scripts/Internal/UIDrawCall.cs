	...
	public void UpdateGeometry (int widgetCount)
	{
		...
		if (count > 0 && (count == uvs.size && count == cols.size) && (count % 4) == 0)
		{
			...
			if (verts.size < 65000)
			{
                ...
                int itr = mMesh.vertexCount / 4;
                Vector2[] uv3 = new Vector2[mMesh.vertexCount];

                for (int i = 0; i < itr; i++)
                {
                    uv3[i * 4 + 0] = new Vector2(0, 0);
                    uv3[i * 4 + 1] = new Vector2(0, 1);
                    uv3[i * 4 + 2] = new Vector2(1, 1);
                    uv3[i * 4 + 3] = new Vector2(1, 0);
                }
