...
//		moneyLabel.text = tweenMoney.ToString("#,##0");
	...
	{
		GameObject [] list = GameObject.FindGameObjectsWithTag("match");
		
		//if(list[0].GetComponent<SpriteRenderer>().sprite.name == list[1].GetComponent<SpriteRenderer>().sprite.name &&
		  // list[1].GetComponent<SpriteRenderer>().sprite.name == list[2].GetComponent<SpriteRenderer>().sprite.name &&
		  // list[0].GetComponent<SpriteRenderer>().sprite.name == list[2].GetComponent<SpriteRenderer>().sprite.name ){
			//Debug.Log("la2eet .....> " + list[0].GetComponent<SpriteRenderer>().sprite.name);
			//MoviesManager.Instance.Match(list[0].GetComponent<SpriteRenderer>().sprite.name);
			//MoviesManager.Instance.disableCam();
		//}
		...
		for (int i = 0; i < 3; i++)
		{
			if(i==1){
				...
				SlotMachineManager.Instance.CheckMatch(lineList[baseX + 0].items[baseY + i].GetTileType(),
				                                       lineList[baseX + 1].items[baseY + i].GetTileType(),
				                                       lineList[baseX + 2].items[baseY + i].GetTileType());
		...
		for (int i = 0; i < lanes; i++)
		{
			...
			for (int j = 0; j < 1; j++)
			{
				...
				Tile tile = lineList[j].items[pos];
				int type = tile.GetTileType();
				if (type != wildIndex)
				{
					firstType = type;
